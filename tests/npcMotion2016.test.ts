...
test("NPC motion speeds use the wire timestamp and native game units", () => {
  ...
  assert.equal(speeds.horizontalSpeed, 2);
  assert.equal(speeds.verticalSpeed, 4);
});
...
test("NPC motion speed calculation handles timestamp wrap and rejects stale samples", () => {
  ...
  assert.equal(wrappedSpeeds.horizontalSpeed, 1 / 0.356);
