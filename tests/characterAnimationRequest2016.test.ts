...
test("Character.PlayAnimation custom strings round-trip the full animal event", () => {
  const object = {
    characterId: GUID,
    ...
    unknownByte1: 0,
    unknownDword2: 1000,
    animationType: "NPC_AttackSpeed",
    unknownByte1xda: 0,
    unknownDword3: 1
  };
  const wire = protocol.pack("Character.PlayAnimation", object);

  assert.ok(wire);
  const parsed = protocol.parse(wire, GatewayChannels.Zone);
  assert.equal(parsed?.name, "Character.PlayAnimation");
  assert.deepEqual(parsed?.data, object);
});
