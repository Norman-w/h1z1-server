# 第六轮重大突破（2026-03-19）- NPC 动画状态机

> 当前最可靠的 NPC 动画驱动结论。**但**：游戏中测试 states6.bit2 无可见效果，怀疑 H1EMU schema 与客户端解析器格式不匹配。

## 核心发现

NPC 动画**不由** PlayAnimation/SeekTarget/ActivateProfile 等包直接驱动，而由 **entity state flags** 驱动。

### 不能直接作为 NPC locomotion 状态的包

| 包名 | opcode | NPC handler | 结果 |
|------|--------|-------------|------|
| Character.PlayAnimation | 0x0f04 | case 3 | vtable+0x630/+0x620 → **NPC 空桩** |
...
- NPC vtable+0x620 → `FUN_14053dc30`：矩阵数学，忽略参数
- NPC vtable+0x630 → `FUN_14091f1f0`：`(void)`，忽略参数
