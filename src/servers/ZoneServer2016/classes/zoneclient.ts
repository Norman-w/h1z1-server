...
import type { TestZombieClockDiagnostics } from "../test-zombie-in-front";
import type { TestZombiePositionReceipt } from "../test-zombie-position-receipt";
...
export class ZoneClient2016 {
  ...
  /** 测试用：是否已安排/执行过前方刷僵尸，避免重复 */
  testZombieSpawned: boolean = false;
  /** Actual inbound movement clock acknowledgement; never set by loading fallbacks. */
  testZombieClockReadyAt?: number;
  /** Bounded experiment diagnostics; no packet payload or environment retention. */
  testZombieClockDiagnostics?: TestZombieClockDiagnostics;
  /** Immutable actual inbound position; never populated from the merged pose cache. */
  testZombiePositionReceipt?: TestZombiePositionReceipt;
  /** Actual valid inbound movement header; never assume the client's version. */
  testZombieMovementVersion?: number;
  testZombieStance?: number;
  testZombieClockResync?: { count: number; requestedAt: number; requestedVersion: number };
  testZombieSynchronization?: { count: number; repliedAt: number; serverTimeU32: number };
  /*(lightWeightNpcQueue: {
