...
import { recordTestZombieClockSample } from "./test-zombie-in-front";
import { recordTestZombiePositionReceipt } from "./test-zombie-position-receipt";
...
/**
 * 首次进世界必须发 WeaponStance + updateEquipment/updateLoadout，否则第三人称无角色模型。
 * 若客户端未正常发送 ClientFinishedLoading，仅靠 isLoading 兜底仍会漏掉这一段 →「进世界看不见自己 + 卡住感」。
 */
function finalizeFirstLoadingIfNeeded(server: ZoneServer2016, client: Client): void {
  if (!client.firstLoading || !client.character) return;
...
/**
 * Keep the AI target set aligned with the clients that are actually in-world.
 * Some 2016 clients finish loading through the server timeout fallback without
 * sending `characterReleased`; the old path cleared loading but never admitted
 * that player to `AiManager.playerEntities`, so production NPCs had no target
 * even though the client was visible and alive.
 */
function ensurePlayerAiRegistration(server: ZoneServer2016, client: Client): void {
  if (!client.character || !client.character.isAlive || client.character.isRespawning) return;
  if (server.aiManager.playerEntities.has(client.character)) return;
  server.aiManager.addEntity(client.character);
  console.log(`[aimanager] player registered for NPC targeting: ${JSON.stringify({
    characterId: client.character.characterId,
    sessionId: client.sessionId,
    source: "client-finished-loading-or-fallback"
  })}`);
}

/** 仅由客户端真实加载完成回执触发；服务器超时兜底不证明客户端已能运行 NPC。 */
function scheduleTestZombieAfterFirstLoading(
  server: ZoneServer2016,
  client: Client
): void {
  if (client.testZombieSpawned) return;
  client.testZombieSpawned = true;
  try {
    require("./test-zombie-in-front").testZombieInFrontOnSpawn(server, client);
  } catch (e) {
    client.testZombieSpawned = false;
    console.error("[test-zombie] 自动刷怪失败", e);
  }
}
...
export class ZonePacketHandlers {
  ...
  ClientIsReady(
    ...
    */
    ...
    // Required for WaitForWorldReady；提前发以免客户端卡 Loading
    ...
    setImmediate(() => server.firstRoutine(client));
    ...
    */
    ...
    // ZoneDoneSendingInitialData 已提前到 NetworkProximityUpdatesComplete 后发送
    ...
    // 兜底：若客户端一直不发 CharacterReleased/Synchronization，3 秒后强制结束 Loading
    setTimeout(() => {
      if (!client?.character || !client.isLoading) return;
      ...
      setTimeout(() => {
        ...
        finalizeFirstLoadingIfNeeded(server, client);
        ensurePlayerAiRegistration(server, client);
  ...
  ClientFinishedLoading(
    ...
    finalizeFirstLoadingIfNeeded(server, client);
    if (packet.data.characterReleased) ensurePlayerAiRegistration(server, client);
    if (client.character.isAlive && !client.character.isRespawning) {
      // The first-loading fallback may have run while the native client was
      // still loading. Start the test encounter only after this real packet.
      if (!client.testZombieSpawned) {
        console.log("[test-zombie] 已收到客户端 ClientFinishedLoading，安排测试遭遇");
      }
      scheduleTestZombieAfterFirstLoading(server, client);
    }
    if (!client.character.isAlive || client.character.isRespawning) {
  ...
  Synchronization(
    ...
    client.testZombieSynchronization = {
      count: (client.testZombieSynchronization?.count ?? 0) + 1,
      repliedAt: Date.now(),
      serverTimeU32: currentTime.getTruncatedU32()
    };
  ...
  async PlayerUpdateManagedPosition(
    ...
    // Explicit replay diagnostic only, before flags0 and normal-NPC discard.
    // Observation failure must never change existing movement handling.
    try { server._devHttpServer?.observeTestNpcIngress(client, packetData); } catch { /* receipt-only */ }
  ...
  PlayerUpdatePosition(
    ...
    let observeAssignment: ((stage: "before" | "after") => void) | undefined;
    try { observeAssignment = server._devHttpServer?.observeTestPlayerMotionIngress(client, packet.data); }
    catch { /* Observation must not affect ordinary ingress handling. */ }
    ...
    // The common clock header is independent of optional gameplay fields.
    // Parser error placeholders must not masquerade as a real zero timestamp.
    recordTestZombieClockSample(
      client, positionUpdate.parseError ? NaN : sequenceTime,
      getCurrentServerTimeWrapper().getTruncatedU32(), flags, positionUpdate.unknown3_int8, positionUpdate.stance
    );
    if (positionUpdate.parseError || flags === 0) {
      return;
    }
    // Stance + rotation still bypasses gameplay updates, but its validated
    // current facing must not be lost before a later same-position baseline.
    if (flags === 513) {
      const finiteVector4 = (value: any) =>
        (Array.isArray(value) || value instanceof Float32Array) &&
        value.length === 4 && Array.from(value).every(Number.isFinite);
      if (client.character?.isAlive && !client.character.isRespawning &&
          client.character.state && finiteVector4(rotation) &&
          finiteVector4(lookAt) && finiteVector4(rotationRaw)) {
    ...
    if (position) {
      ...
      try { observeAssignment?.("before"); } catch { /* Diagnostic only. */ }
      client.character.state.position = position;
      try { observeAssignment?.("after"); } catch { /* Diagnostic only. */ }
    ...
    // Record only this packet's fields, after applying them. The accumulated
    // positionUpdate/state cache is not evidence that a new packet had position.
    recordTestZombiePositionReceipt(
      client, positionUpdate, getCurrentServerTimeWrapper().getTruncatedU32()
    );
