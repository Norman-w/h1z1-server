...
export class WorldObjectManager {
  ...
  createNpc(
    ...
    // Production chase uses the native seek rail as a target/acceleration hint
    // and one server-authored PlayerUpdatePosition stream for visible motion.
    // The server advances the same shadow position that it publishes. The
    // bounded `/ztest` creator sets its own explicit A/B flags, so the seek-only
    // negative control cannot silently change the production factory.
    if (
