...
export const commands: Array<Command> = [
  ...
  {
    name: "ztest",
    description: "Local repeatable zombie observation: flat, slope, stop, status",
    permissionLevel: PermissionLevels.ADMIN,
    execute: (server: ZoneServer2016, client: Client, args: Array<string>) => {
      if (!server._devHttpServer) {
        server.sendChatText(client, "[ztest] Local developer test service is not enabled.", true);
        return;
      }
      server._devHttpServer.handleTestZombieCommand(client, args);
    }
  },
  {
    name: "despawnztest",
    description: "Developer probe: remove the automatic test zombie without changing replay state",
    permissionLevel: PermissionLevels.ADMIN,
    execute: (server: ZoneServer2016, client: Client) => {
      server.removeTestZombie();
      server.sendChatText(client, "[despawnztest] automatic test zombie removed", true);
    }
  },
  {
    name: "prodseektest",
    description: "Developer probe: publish one NPC from WorldObjectManager.createNpc and enable ordinary AI",
    permissionLevel: PermissionLevels.ADMIN,
    execute: (server: ZoneServer2016, client: Client, args: Array<string>) => {
      const aliases: Record<string, number> = {
        zombie: ModelIds.ZOMBIE_MALE_WALKER,
        zombie_male: ModelIds.ZOMBIE_MALE_WALKER,
        zombie_female: ModelIds.ZOMBIE_FEMALE_WALKER,
        screamer: ModelIds.ZOMBIE_SCREAMER,
        bear: ModelIds.BEAR
      };
      const key = String(args[0] ?? "zombie").trim().toLowerCase();
      const modelId = aliases[key] ?? (key !== "" ? Number(key) : NaN);
      ...
      const distance = args[1] === undefined ? 9 : Number(args[1]);
      if (!Number.isInteger(modelId) || !allowed.has(modelId) || !Number.isFinite(distance) ||
          distance < 6 || distance > 12) {
        server.sendChatText(client,
          "[prodseektest] Usage: /prodseektest zombie|zombie_female|screamer|bear [6-12]", true);
        return;
      }
      if (!client.character?.isAlive || client.character.isRespawning || client.isLoading ||
          Object.values(server._clients).length !== 1 || server._clients[client.sessionId] !== client) {
        server.sendChatText(client, "[prodseektest] Enter the world alive in solo mode first.", true);
        return;
      }

      // Keep the production-factory probe separate from the bounded /ztest
      // lease. Both old probes are removed so one NPC is visible at a time.
      server.removeTestZombie();
      server.removeTestSpeciesProbe();
      const playerPosition = client.character.state.position;
      const heading = quat2heading(client.character.state.rotation);
      const headingRad = (heading / 255) * 2 * Math.PI;
      const position = new Float32Array([
        playerPosition[0] + Math.sin(headingRad) * distance,
        playerPosition[1],
        playerPosition[2] + Math.cos(headingRad) * distance,
        1
      ]);
      const npc = server.worldObjectManager.createNpc(
        ...
        modelId,
        position,
        new Float32Array(client.character.state.rotation),
        0
      );
      // The factory has already selected clientDrivenSeek for chase-capable
      // models. Hold the AI until the same lightweight/full publication used by
      // the world visibility path has reached this client.
      server.aiManager.removeEntity(npc);
      npc.setFacingToward(playerPosition);
      server._testSpeciesProbeNpcCharacterId = npc.characterId;
      client.spawnedEntities.add(npc);
      server.addLightweightNpc(client, npc, npc.nameId);
      server.sendStandardFullNpcInit(client, npc);
      server.sendChatText(client,
        `[prodseektest] factory NPC published model=${modelId}, npc=${npc.characterId}, ` +
        `profileId=${npc.profileId}, clientDrivenSeek=${npc.clientDrivenSeek}; ` +
        "AI starts in 1.5s. Use /despawnnpcvisible when done.", true);
      console.log(`[prodseektest] ${JSON.stringify({
      ...
      const timer = setTimeout(() => {
        if (server._testSpeciesProbeNpcCharacterId !== npc.characterId ||
            server._npcs[npc.characterId] !== npc ||
            server._clients[client.sessionId] !== client ||
            !client.character.isAlive || client.character.isRespawning) return;
        server.aiManager.addEntity(npc);
        npc.sendIdleStance();
        server.sendChatText(client,
          `[prodseektest] ordinary AI enabled for ${npc.characterId}; observe native seek/attack transition.`, true);
        console.log(`[prodseektest] ordinary AI enabled ${JSON.stringify({
          characterId: npc.characterId,
          clientDrivenSeek: npc.clientDrivenSeek,
          isAlive: npc.isAlive,
          npcEntities: server.aiManager.npcEntities.size,
          playerEntities: server.aiManager.playerEntities.size,
          serverPosition: Array.from(npc.state.position),
          playerPosition: Array.from(client.character.state.position)
        })}`);
      }, 1500);
      timer.unref?.();
      server._testSpeciesProbeTimer = timer as unknown as ReturnType<typeof setInterval>;
    }
  },
  {
    name: "spawnnpcvisible",
    description: "Developer probe: show one animal through the normal lightweight/full NPC path",
    permissionLevel: PermissionLevels.ADMIN,
    execute: (server: ZoneServer2016, client: Client, args: Array<string>) => {
      const aliases: Record<string, number> = {
        deer: ModelIds.DEER,
      ...
      const modelId = aliases[key] ?? (key !== "" ? Number(key) : NaN);
      const allowed = new Set([ModelIds.DEER, ModelIds.DEER_BUCK, ModelIds.WOLF, ModelIds.BEAR]);
      const distance = args[1] === undefined ? 8 : Number(args[1]);
      if (!Number.isInteger(modelId) || !allowed.has(modelId) || !Number.isFinite(distance) ||
          distance < 6 || distance > 12) {
        server.sendChatText(client, "[spawnnpcvisible] Usage: /spawnnpcvisible deer|deer_buck|wolf|bear [6-12]", true);
        return;
      }
      if (!client.character?.isAlive || client.character.isRespawning || client.isLoading ||
          Object.values(server._clients).length !== 1 || server._clients[client.sessionId] !== client) {
        server.sendChatText(client, "[spawnnpcvisible] Enter the world alive in solo mode first.", true);
        return;
      }
      const playerPosition = client.character.state.position;
      ...
      // Npc construction registers with AiManager; the probe is deliberately static.
      server.aiManager.removeEntity(npc);
      server._npcs[characterId] = npc;
      server._testSpeciesProbeNpcCharacterId = characterId;
      client.spawnedEntities.add(npc);
      server.addLightweightNpc(client, npc, npc.nameId);
      server.sendStandardFullNpcInit(client, npc);
      server.sendChatText(client,
        `[spawnnpcvisible] spawned model=${modelId} npc=${characterId} profileId=${npc.profileId} npcDefinitionId=${npc.npcDefinitionId}; static probe only. Use /despawnnpcvisible when done.`,
        true);
      console.log(`[spawnnpcvisible] ${JSON.stringify({ characterId, modelId, profileId: npc.profileId, npcDefinitionId: npc.npcDefinitionId, position: Array.from(position) })}`);
    }
  },
  {
    name: "despawnnpcvisible",
    description: "Developer probe: remove the visible animal test NPC",
    permissionLevel: PermissionLevels.ADMIN,
    execute: (server: ZoneServer2016, client: Client) => {
      server.removeTestSpeciesProbe();
      server.sendChatText(client, "[spawnnpcvisible] probe removed", true);
    }
  },
  {
    name: "movetestnpcvisible",
    description: "Developer probe: move the visible animal with server position packets, optionally with the native chase state",
    permissionLevel: PermissionLevels.ADMIN,
    execute: (server: ZoneServer2016, client: Client, args: Array<string>) => {
      const id = server._testSpeciesProbeNpcCharacterId;
      const npc = id ? server._npcs[id] : undefined;
      const seconds = args[0] === undefined ? 4 : Number(args[0]);
      const locomotionMode = String(args[1] ?? "position").trim().toLowerCase();
      const sendChaseState = locomotionMode === "graph" || locomotionMode === "locomotion";
      if (!id || !npc || !Number.isFinite(seconds) || seconds < 2 || seconds > 8 ||
          !["position", "graph", "locomotion"].includes(locomotionMode) ||
          !client.character?.isAlive || client.character.isRespawning || client.isLoading ||
          Object.values(server._clients).length !== 1 || server._clients[client.sessionId] !== client) {
        server.sendChatText(client, "[movetestnpcvisible] Usage: /movetestnpcvisible [2-8] [position|graph], with a visible probe in solo world.", true);
        return;
      }
      const probeId = id;
      server.stopTestSpeciesProbeMovement();
      const target = client.character.state.position.slice();
      const stopDistance = 2.75;
      const stepDistance = 2.5 * 0.15;
      const startedAt = Date.now();
      npc.behaviorState = 1;
      try { npc.sendIdleStance(); } catch (error) {
        server.sendChatText(client, `[movetestnpcvisible] initial idle failed: ${String(error)}`, true);
        npc.behaviorState = 0;
        return;
      }
      if (sendChaseState) {
        try { npc.sendLocomotionState(1); } catch (error) {
          server.sendChatText(client, `[movetestnpcvisible] chase state failed: ${String(error)}`, true);
          npc.behaviorState = 0;
          try { npc.sendIdleStance(); } catch { /* best effort */ }
          return;
        }
      }
      const finish = (reason: string) => {
        server.stopTestSpeciesProbeMovement();
        if (sendChaseState) {
          try { npc.sendLocomotionState(0); } catch (error) {
            console.error("[movetestnpcvisible] failed to clear probe locomotion state", error);
          }
        }
        if (server._clients[client.sessionId] === client) {
          server.sendChatText(client, `[movetestnpcvisible] finished: ${reason}; mode=${sendChaseState ? "graph+position" : "position-only"}.`, true);
        }
        console.log(`[movetestnpcvisible] ${JSON.stringify({ id, modelId: npc.actorModelId, profileId: npc.profileId, reason, locomotionMode: sendChaseState ? "graph" : "position", elapsedMs: Date.now() - startedAt, finalPosition: Array.from(npc.state.position) })}`);
      };
      const timer = setInterval(() => {
        if (server._testSpeciesProbeNpcCharacterId !== probeId || server._npcs[probeId] !== npc ||
            server._clients[client.sessionId] !== client || !client.character.isAlive || client.character.isRespawning) {
          finish("owner_or_probe_changed");
          return;
        }
        ...
        const current = npc.state.position;
        const dx = target[0] - current[0], dz = target[2] - current[2];
        const distance = Math.hypot(dx, dz);
        if (!Number.isFinite(distance) || distance <= stopDistance) {
          finish("contact_range");
          return;
        }
        const step = Math.min(stepDistance, distance - stopDistance);
        const next = new Float32Array([current[0] + dx / distance * step, current[1], current[2] + dz / distance * step, 1]);
        npc.setFacingToward(target);
        npc.goTo(next, true);
      }, 150);
      timer.unref?.();
      server._testSpeciesProbeTimer = timer;
      server.sendChatText(client, `[movetestnpcvisible] moving model=${npc.actorModelId} for up to ${seconds}s; do not move.`, true);
    }
  },
  {
    name: "seektestnpcvisible",
    description: "Developer probe: install the native Character.SeekTarget rail on the visible animal",
    permissionLevel: PermissionLevels.ADMIN,
    execute: (server: ZoneServer2016, client: Client, args: Array<string>) => {
      const id = server._testSpeciesProbeNpcCharacterId;
      const npc = id ? server._npcs[id] : undefined;
      ...
      if (!id || !npc || !Number.isFinite(seconds) || seconds < 2 || seconds > 8 ||
          !client.character?.isAlive || client.character.isRespawning || client.isLoading ||
          Object.values(server._clients).length !== 1 || server._clients[client.sessionId] !== client) {
        server.sendChatText(client, "[seektestnpcvisible] Usage: /seektestnpcvisible [2-8], with a visible probe in solo world.", true);
        return;
      }
      server.stopTestSpeciesProbeMovement();
      const target = client.character.state.position.slice();
      ...
      npc.seekTarget(client.character.characterId, target);
      const finish = (reason: string) => {
        if (server._testSpeciesProbeNpcCharacterId !== id || server._npcs[id] !== npc) return;
        npc.clearMovementController(true);
        npc.behaviorState = 0;
        try { npc.sendLocomotionState(0); } catch { /* diagnostic cleanup */ }
        try { npc.sendIdleStance(); } catch { /* diagnostic cleanup */ }
        if (server._clients[client.sessionId] === client)
          server.sendChatText(client, `[seektestnpcvisible] finished: ${reason}.`, true);
        console.log(`[seektestnpcvisible] ${JSON.stringify({ id, modelId: npc.actorModelId, profileId: npc.profileId, reason, elapsedMs: Date.now() - startedAt, serverPosition: Array.from(npc.state.position) })}`);
      };
      const timer = setTimeout(() => finish("duration"), seconds * 1000);
      timer.unref?.();
      server._testSpeciesProbeTimer = timer as unknown as ReturnType<typeof setInterval>;
      server.sendChatText(client, `[seektestnpcvisible] native seek rail installed for ${seconds}s; do not move.`, true);
    }
  },
  {
    name: "console",
